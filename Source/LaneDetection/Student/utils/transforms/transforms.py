...
class Resize(CustomTransform):
    ...
    def __call__(self, sample):
        ...
        segLabel = sample.get('seg_label', None)
...
class addNoise(CustomTransform):
    ...
    def __call__(self, sample):
        img = sample.get('image')
        mean = 0
        gauss = np.random.normal(mean, self.factor, img.shape)
