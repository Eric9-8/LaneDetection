...
def parse_args():
    ...
    parser.add_argument("--logdir", type=str, default='./new_log/StudentV1_n')
    parser.add_argument("--weight_dir", type=str, default='/home/arc-cjl2902/LaneDetection2/Student/exp/v1_n')
    parser.add_argument("--data_dir", type=str, default='/home/arc-cjl2902/Dataset/50_CULaneDataset')
    parser.add_argument("--round", type=str, default="n")
...
#                           collate_fn=train_dataset.collate, sampler=train_sample, num_workers=16)
# Full train dataset
...
model = NET(backbone='regnet_y_400mf', pretrained=True)
...
def train(epoch):
    ...
    if epoch % 1 == 0:
        ...
        save_name = os.path.join(args.weight_dir + '/StudentV1_{}'.format(args.round) + '.pth')
...
def valid(epoch):
    ...
    if val_loss < best_val_loss:
        ...
        save_name = os.path.join(args.weight_dir + '/StudentV1_{}'.format(args.round) + '.pth')
        copy_name = os.path.join(args.weight_dir + '/StudentV1_{}'.format(args.round) + '_best.pth')
...
def main():
    ...
    if args.resume:
        ...
        save_dict = torch.load(os.path.join(args.weight_dir + '/StudentV1_{}'.format(args.round) + '.pth'))
        if isinstance(model, torch.nn.DataParallel):
