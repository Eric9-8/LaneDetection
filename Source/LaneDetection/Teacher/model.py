from Teacher.unet_T import UnetHead
...
class _TNET(nn.Module):
    def __init__(self, pretrained=True, backbone=None):
        ...
        self.layer1 = nn.Sequential(nn.Dropout(0.2), nn.Conv2d(72, 2, 1))
        # self.layer2 = nn.Sequential(nn.Softmax(dim=1), nn.AvgPool2d(2, 2))
        # self.fc = nn.Sequential(nn.Linear(self.fc_input_feature, 128), nn.ReLU(inplace=True), nn.Linear(128, 4))
